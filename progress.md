...
- **추가 (같은 세션)**: 시간 범위 카드(예 8:15-9:15)의 정렬 어긋남 수정. 범위 카드에만 있던
  "위쪽 정렬 + 4px 내리기" 예외 규칙 때문에 글자·+는 위, 체크박스·손잡이는 아래로 들쭉날쭉했음.
  예외를 없애고 보통 카드처럼 세로 가운데 정렬(믹스 `.cardRange` / 리스트 `.itemRange`).
  검증: 믹스·리스트 모두 시간·체크박스·글자·+·손잡이의 세로 중심 좌표가 카드 중심과 일치,
  110 tests · lint · build 통과.
- **추가 2 (같은 세션)**: 시간 범위 카드의 시간 글자 위아래 여백이 4.5px로 너무 좁던 문제 보완.
  카드 높이가 보통 카드(34px)와 같은데 시간이 두 줄이라 꽉 찼던 것 → 범위 카드의 시간 묶음에
  위아래 padding 6px 추가(믹스 `.cardRange .timeWrap` / 리스트 `.itemRange .timeWrap`).
  결과: 여백 4.5 → 9px(카드 34 → 43px). 보통 카드 11.5px와 비슷한 여유. 글이 줄바꿈된 범위 카드는
  글 높이로 카드가 커져 11.9~13.9px. 지난번 정렬 수정 때 남은 옛 "상단 정렬" 주석도 정리.
  110 tests · lint · build 통과.
- **추가 3 (같은 세션)**: 자식(하위) 카드의 글자 폭을 부모 카드에 맞춤. 자식엔 "+" 버튼이 없어 글자가
  오른쪽으로 26~30px 더 뻗고, 체크박스 칸이 4px 좁아 글자가 4px 먼저 시작했음.
  · 믹스: 자식 글자·수정칸에 + 자리(30px) 비움, 체크박스 칸 20→24px, 오른쪽 padding 12→8px
    (손잡이도 부모와 같은 세로줄). 리스트: 자식 글자에 30px 비움, 체크박스 칸 20→24px.
  · 검증(375px): 믹스 글자 102~295 / 체크 원 중심 80 / 손잡이 335~355, 리스트 글자 98~287 /
    체크 중심 76 — 부모·자식 모두 동일. 자식 수정칸도 102~295. 110 tests · lint · build 통과.
  · 참고: 믹스 '시간 미지정' 카드는 시간 칸과 + 버튼이 없는 별도 배치라 글자 구간(64~313)이 다름 — 이번 범위 밖.
- **추가 4 (같은 세션)**: 일정카드 위아래 여백 통일. 기준값 `--card-gap-y: 8px`(global.css)를 새로 두고
  (범위 카드 9px의 0.9배), 카드가 커지는 모든 경우(시간 범위 두 줄 / 글 줄바꿈 / 하위 / 시간 미지정,
  믹스·리스트 모두)에서 카드 테두리 ~ 눈에 보이는 글자까지 거리가 8px이 되게 함.
  · 계산: 카드 안쪽 여백 3px + 글자/시간 묶음 padding. 글자는 줄간격 1.3의 보이지 않는 여백
    0.15em을 빼서(`--card-text-pad-y`), 시간(줄간격 1)은 그대로(`--card-time-pad-y`).
  · 믹스 하위 카드 안쪽 여백 5→3px(한 줄일 땐 min-height 32px가 높이 결정 → 변화 없음).
    리스트 하위 카드는 손잡이 높이 때문에 안쪽 2px 유지, 글자 쪽에 +1px 보정.
  · 검증(375px, 16개 경우): 줄바꿈·범위 카드 전부 위/아래 8/8px. 한 줄 카드는 크기 불변
    (보통 34px·여백 11.5 / 하위 32px·10.5). 110 tests · lint · build 통과.
- **추가 5 (같은 세션)**: 믹스 보기 체크 원을 세로 시간선 정중앙에 정렬. 원 중심(80px)이 선(76px)보다
  4px 오른쪽이었음(믹스 카드 왼쪽 안쪽 여백이 리스트보다 4px 넓었던 것).
  · 체크박스만 옮기지 않고 시간·체크·글자 묶음 전체를 4px 왼쪽으로: 카드 padding-left 10→6px,
    하위 카드 체크박스 margin-left 12→8px. 세로선 계산 주석도 실제 값(76px)으로 갱신.
  · 검증(375px): 선 76 = 원 중심 76(시간·범위·하위 카드 모두), 원 양옆 간격 15/15 유지,
    부모·자식 글자 구간 98~295 동일. 범위 카드 "-22:15"~카드 왼쪽 끝 10.4→6.4px.
    '시간 미지정' 카드는 세로선 밖이라 그대로. 110 tests · lint · build 통과.
- **추가 6 (같은 세션)**: 시간 글자를 '카드 왼쪽 끝 ~ 체크 원 왼쪽 끝' 공간의 정가운데로 이동(믹스·리스트).
  · 시간 칸을 그 공간 전체 폭(믹스 57px / 리스트 61px)으로 넓히고 음수 margin으로 차지하는 폭은
    기존 36px 유지 → 체크박스·글자 위치 불변. 범위 시간은 두 줄을 한 덩어리로 가운데, 줄끼리는 오른쪽 끝 맞춤(grid).
  · 검증(375px): 양옆 거리 믹스 한 줄 12.8/12.8 · 범위 10.7/10.7, 리스트 한 줄 14.8/14.8 · 범위 12.7/12.7.
    체크 원 76 · 글자 구간 불변, 체크박스 왼쪽 끝 클릭도 체크박스로 전달. 110 tests · lint · build 통과.
  · 참고: 테스트 중 미리보기 데이터의 한 일정 시간이 20:00→12:00(720, '오후' 구분선 위치)으로 바뀐 걸 발견.
    화면 크기·보기 전환 반복으로 재현 안 됨 → 미리보기 창에서 카드가 끌린 것으로 추정, 코드 변경과 무관.
- **추가 7 (같은 세션)**: 카드 오른쪽 "+"(하위 일정 추가)와 손잡이(=)의 세로 중앙 통일.
  버튼 칸은 둘 다 정중앙이었지만 "+"가 글꼴 글자라 글꼴 모양 때문에 약 1.1px 아래로 그려졌음
  (아이폰 캡처에서도 확인). 기기마다 글꼴이 달라 숫자 보정 대신, 손잡이처럼 선 두 개로 직접 그리는
  `.plusIcon`(10×10px, 굵기 1.5px = 기존 글자 크기·손잡이 선 굵기)으로 교체(믹스·리스트).
  검증(375px): 모든 카드에서 "+"·손잡이·체크 원 중심 = 카드 중심(차이 0), 누르면 파란색 선택 정상.
  110 tests · lint · build 통과.

## 2026-09-19 (12세션: 빈 시간 구간 드래그 개선 — 브랜치 feat/drag-empty-hours)
- **문제**: 오후·저녁·자정 구분선 사이에 일정이 없으면 6시간이 20px 남짓에 몰려 있어, 드래그로
  원하는 시간에 놓기가 거의 불가능(1px에 약 20분).
- **결정(오너)**: 노션처럼 칸에 착 붙는 사용감 / 1시간 칸(정각에 자석, 사이는 5분 단위) /
  범위는 오늘=지금 이후 정각, 내일=아침 6시~, 둘 다 밤 11시 줄까지.
- **Done**:
  · `timelineMath.ts`: `calcEmptyHourSlots`(그 1시간 안에 일정이 없는 정각만, 구분선 정각 제외),
    `buildSegments`(훅 안의 타임라인 순서 코드를 꺼내 테스트 가능하게 — 구분선·현재시각·정각 줄을
    시간순으로 섞음), `calcUnscheduledDropTime`(시간 미지정 카드도 카드 드래그와 같은 계산 + 기준점).
    테스트 12개 추가(110 → 122).
  · 훅: 카드/시간 미지정 카드를 집는 순간 빈 정각 줄(44px)을 펼치고, 그려진 직후(useLayoutEffect)
    ① 잡은 카드가 밀린 만큼 스크롤 보정(여유가 모자라면 맨 아래 보이지 않는 빈 공간을 그만큼 늘림)
    ② 정각 줄을 포함해 기준점 위치를 다시 잼. 끄는 동안 화면 위/아래 끝 64px 안이면 자동 스크롤
    (스크롤된 양만큼 손가락 위치 보정). 놓으면 칸·빈 공간 원상 복구.
  · 믹스 화면: 정각 줄(`.dragHourRow`, 점선) 렌더. 드래그 중엔 'N시간 생략' 대신 정각 줄.
- **검증(개발 서버 375px, 합성 포인터 이벤트)**: 집는 순간 카드가 손가락 밑 그대로(스크롤 보정 확인),
  15:00 줄 → 15:00 · 줄+6px → 15:00(자석) · 15~16 가운데 → 15:30 · 16:00 줄-10px → 15:55,
  놓으면 960 저장 + 칸 접힘. 시간 미지정 카드 → 22:00 저장. 자동 스크롤(위 끝에서 0.1초당 약 72px,
  시간 따라 갱신). 시간 있는 일정이 0개여도 7~14시 줄 모두 정확. 오늘 탭은 지금 이후 정각만.
  제자리에 놓으면 시간 불변, 옆으로 밀어 하위 편입 기존 기능 정상. 콘솔 에러 0(새로 불러온 화면 기준).
  122 tests · lint · build 통과.
- **발견·수정**: 첫 구현에서 시간 미지정 카드를 잡자마자 타임라인에 들어가 버림(스크롤 여유 280px
  부족으로 보정 실패) → 드래그 중에만 맨 아래 빈 공간 추가로 해결.
- **Note**:
  · 타임라인 순서 변경: 현재시각 표시가 이제 시간순으로 들어감(예: 11:40이면 '오후' 구분선보다 위).
    예전엔 구분선 뒤에 붙는 경우가 있었음(시간 계산 기준점 순서가 뒤집히는 문제도 함께 해소).
  · 하위 일정을 끌어 상위로 올리는 드래그는 이번 범위 밖(기존 방식 유지).
  · 실제 손가락 감각(정각 자석 세기·자동 스크롤 속도)은 폰에서 확인 필요. 조절값: 줄 높이 44px
    (MixView.module.css), AUTO_SCROLL_ZONE 64 / AUTO_SCROLL_MAX 14(useTimelineInteractions.ts).
- **Next**: 오너 폰 확인 → 필요시 줄 높이·스크롤 속도 조절 → main 병합·푸시
  (feat/edit-full-text 위에서 갈라진 브랜치라 그 커밋 8개도 함께 포함됨).
- **추가 (같은 세션)**: 오너 피드백 "펼쳐지는 길이가 너무 김, 절반 감도" → 정각 줄 높이 44 → 22px.
  줄이 좁아진 만큼 정각 자석 구간도 ±8 → ±4px로 좁힘(기준점별 `snapZone`, 카드·구분선은 8px 유지).
  측정(21~22시, 22px): ±3px → 21:00 · +5px → 21:05 · 가운데 → 21:30 · 다음 줄 -5px → 22:00.
  **버그 수정**: 내용이 화면보다 짧을 때 보정용 빈 공간 일부가 화면 안 빈자리를 채우는 데 쓰여
  스크롤 보정이 63px 모자랐음(잡은 카드가 손가락에서 벗어나고, 제자리에 놓아도 시간이 바뀜) →
  빈 공간을 늘린 뒤 다시 재서 모자라면 더 늘리도록 수정. 세 카드 모두 잡는 순간 차이 0px 확인.
  테스트 3개 추가(122 → 125). lint · build 통과.
- **추가 2 (같은 세션)**: 오너 요청 "한 칸 30px" → 정각 줄 높이 22 → 30px, 정각 자석 구간 ±4 → ±5px
  (지금까지 비율 유지). 줄 사이 20px에서 5분 단위(5분 ≈ 1.7px).
  측정(내일 탭 9~10시): +4px → 9:00 · +6 → 9:05 · +10 → 9:15 · 가운데 → 9:30 · +20 → 9:45 ·
  10시 줄 -6px → 9:55 · -4px → 10:00.
  **버그 수정**: 일정이 많아 화면이 스크롤되는 상태에서도 보정용 빈 공간이 모자라던 문제
  (필요 105px인데 "모자란 만큼 × 최대 3번" 방식이라 90px에서 멈춤, 카드가 15~30px 벗어남) →
  빈 공간의 실제 끝 위치로 전체 내용 높이를 재서 필요한 양을 한 번에 계산. 오늘 탭 8개 카드 모두 0px.
  미리보기에 오너 실제 일정(기상·헬스·카공…)이 있어 데이터는 건드리지 않고 측정(제자리 놓기 +
  내일 탭 임시 일정은 백업으로 원상 복구 확인). 125 tests · lint · build 통과.

## 2026-09-19 (13세션: 주요 일정(별) — 브랜치 feat/star-important)
- **요청**: 하단 입력칸 안 오른쪽 끝에 별 버튼. 켜 두고 등록하면 그 일정의 체크 동그라미가 노란 별.
- **결정(오너)**: 등록 후 별 버튼 자동 꺼짐 / 미완료 = 노란색 채운 별, 완료 = 회색 별 /
  이번엔 등록할 때만 지정(나중에 켜고 끄기는 추후, 글자 수정해도 별 유지).
- **Done**:
  · `Todo.starred?: boolean` (켰을 때만 저장 → 기존 데이터 변환 불필요). `addTodo(..., starred)`.
    내일로 미루기·날짜 넘김은 기존처럼 일정 정보를 통째로 복사해 별도 유지. 테스트 5개(125 → 130).
  · `StarIcon.tsx` 공용 별 아이콘(노랑 #F5B800 / 완료 회색 #9CA3AF).
  · 입력칸: 안쪽 오른쪽 끝 별 버튼(꺼짐 = 회색 테두리, 켜짐 = 노란 별), 누를 때 입력칸 포커스
    유지, 등록하면 자동 꺼짐. 입력 글자가 버튼 밑으로 안 들어가게 오른쪽 여백 34px.
  · 카드 4곳(믹스 시간 카드·시간 미지정·하위 일정·리스트)에서 starred면 동그라미/네모 대신 별.
- **검증(375px)**: 별 켜고 등록 → 노란 별 카드 + 버튼 자동 꺼짐 + 입력칸 포커스 유지, 저장값
  starred:true. 별 중심 = 세로 시간선(76px)·카드 세로 중앙(차이 0), 하위 일정 별도 동일.
  완료 → 회색 별, 리스트 보기도 별 표시. 미리보기의 오너 실제 일정은 백업 후 테스트하고 원상 복구 확인.
  130 tests · lint · build 통과.
- **Note**: 앞 메시지에서 계획만 보여 드려 "별 버튼이 안 보인다"는 문의가 있었음(코드 작성 전이었음).
  템플릿 저장/적용 시 별은 따라가지 않음(범위 밖).
- **Next**: 폰 확인 → 필요시 등록 후 별 켜고 끄기 방법 결정.
  브랜치 쌓임: feat/edit-full-text → feat/drag-empty-hours → feat/star-important.
- **추가 (같은 세션)**: 이미 등록한 일정도 별 켜고 끄기. 일정을 눌러 글자를 고치는 동안에만 카드
  오른쪽에 별 버튼(`StarToggleButton`)이 나타남 — 믹스 시간 카드는 "+" 자리, 시간 미지정·하위 일정은
  손잡이 왼쪽, 리스트는 들여쓰기 버튼 옆. 누를 때 수정칸 포커스를 뺏지 않아 수정 상태 유지.
  스토어 `toggleStar`(끄면 표시 자체 제거, 되돌리기 가능) + 테스트 3개(130 → 133).
...
## 2026-09-19 (14세션: 링크로 일정 공유 — 브랜치 feat/share)
- **요청**: URL에 데이터를 담는 방식(코딩애플 영상 참고)으로 일정 공유. 조사 후 추천 → 진행.
  영상은 Gemini 브라우저 사용 중·Gemini CLI 지원 종료·자막 차단으로 직접 확인 못함(제목·설명만 확인).
- **결정(오너)**: 오늘/내일 중 지금 탭에서 일부만 골라 공유 / 완료 체크도 포함 / 받는 쪽은 보기 전용만
  (내 일정에 추가 없음) / 템플릿 제외 / 버튼 = 링크 아이콘, 박수 오른쪽.
- **Done**:
  · `lib/shareCodec.ts`: 고른 일정 → 시간순 공유 목록, 브라우저 기본 압축(deflate-raw)+base64url로
    `#share=...` 링크. 받은 값은 하나씩 검사(망가진 링크 → null). 날짜·요약 글 도우미. 테스트 10개(133 → 143).
  · `SharePanel`: 아래서 올라오는 선택 창(기본 전체 체크, 큰 일정 끄면 하위도 꺼짐, 하위 켜면 부모도 켜짐),
    "N개 공유하기" → 휴대폰 공유창(글 요약 + 링크), 공유창 없으면 복사. 링크는 체크 바뀔 때 미리 만들어 둠
    (아이폰은 누른 순간 공유창을 열어야 해서).
  · `SharedView` + `useSharedLink`: 링크로 들어오면 보기 전용 화면(실제 날짜, 완료·별·시간 범위·하위 일정),
    "내 일정으로 돌아가기"는 주소의 #share만 지우고 원래 화면으로.
- **검증(375px)**: 내일 탭 실제 일정 11개 중 2개 빼고 공유 → 링크 약 200자, 받는 화면에 9개 정확히 표시.
  완료·범위·긴 글 테스트 링크 확인(긴 글 옆 별 찌그러짐 발견 → 수정). 망가진 링크 안내, 돌아가기 후 주소 정리.
  오너 저장 데이터는 백업과 완전히 동일(공유 기능은 저장값을 바꾸지 않음). 143 tests · lint · build 통과.
- **Note**: 실제 휴대폰 공유창(카톡 등)은 미리보기에서 열 수 없어 가짜로 바꿔 전달값만 확인함 → 폰 확인 필요.
  압축 기능은 iOS 16.4 이상. 링크를 가진 사람은 누구나 볼 수 있음.
- **Next**: 폰에서 공유창·카톡 링크 열기 확인 → 괜찮으면 main 병합·배포(오너 요청 시).
